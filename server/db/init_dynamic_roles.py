#!/usr/bin/env python3
"""
Initialize dynamic roles and permissions in the database.

This script creates the initial set of dynamic roles and permissions
based on the constants defined in server/constants/permissions.py.

Usage:
    python -m server.db.init_dynamic_roles
"""
...
def migrate_role_permissions(
    session: Session, role_mapping: dict, permission_mapping: dict
):
    ...
    for role_value, perm_codes in ROLE_PERMISSION_MATRIX.items():
        ...
        for perm_code in perm_codes:
            ...
            existing = (
                ...
                .filter_by(role_id=role_id, permission_id=perm_id)
                .first()
            )
            ...
            new_rel = RolePermission(role_id=role_id, permission_id=perm_id)
            session.add(new_rel)
