...
def update_user(
    ...
    if not target:
        raise HTTPException(status_code=404, detail="User not found.")
    if target.id == current_user.id and body.role_id is not None and body.role_id != current_user.role_id:
...
def update_user_role(
    ...
    try:
        target.role_id = body.role_id
        db.commit()
        return OkResponse(message=f"User {target.username} role updated to '{role_obj.name}'.")
    except HTTPException:
        raise
    except Exception as exc:
        db.rollback()
